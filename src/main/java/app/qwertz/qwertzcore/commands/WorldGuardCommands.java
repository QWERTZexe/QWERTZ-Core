...
public class WorldGuardCommands implements CommandExecutor {
    ...
    public static boolean enableHighFrequencyFlags() {
        ...
        try {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "wg reload"); // I was unable to reload the config with the API (there's no way to do it with API i'm pretty sure)
            return true;
        } catch (IOException e) {
            System.out.println("We were unable to enable 'high-frequency-flags' option in the WorldGuard config. /flow will be broken. ");
            return false;
        }
    }
